...
class Gating(nn.Module):
    ...
    def forward_int(self, y):
        ...
        for i, layer in enumerate(self.net[:-1]):
            ...
            y = self.activation(y)
        y = self.net[-1](y)
        y=torch.sin(1*pi*y)+1
        # y= self.softmax(y)+1e-2
        return y
...
class MOE_dense_weight(nn.Module):   
...
@torch.no_grad()
def moew_penultimate_getter(model, x):
    moe = model.model
    feats = []
    weights = []
    ...
    for e, expert in enumerate(moe.experts):
        ...
        # 把 w 重复到和 h 同样的 feature 维度
        weights.append(w.expand(-1, h.shape[1]))   # [N, H]  (不复制内存)
        break

    Phi = torch.cat(feats, dim=1)           # [N, E*H]
    Weight_big = torch.cat(weights, dim=1)  # [N, E*H]
    return Phi