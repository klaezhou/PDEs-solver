...
def parse_args():
    ...
    # --- basic config ---
    parser.add_argument("--eq", type=str, default="heat_ms2d")
    ...
    # Heat2D-MS input: (x, y, t)
    parser.add_argument("--input_dim", type=int, default=3)
    parser.add_argument("--bump_depth", type=int, default=1)

    # centers only in spatial dimensions: (x, y), not time
    parser.add_argument("--center_mins", type=list, default=[0.0, 0.0])
    ...
    parser.add_argument("--Ni", type=int, default=2000)
    parser.add_argument("--Nb", type=int, default=2000)
    ...
    # error / ground truth plot at fixed time slice
    parser.add_argument("--eval_grid_n", type=int, default=200)
    parser.add_argument("--t_plot", type=float, default=5.0)

    # --- loss weights ---
    parser.add_argument("--w_pde", type=float, default=1.0)
    parser.add_argument("--w_ic", type=float, default=5000.0)
    parser.add_argument("--w_bc", type=float, default=5000.0)
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default="/home/zhy/Zhou/DLPDEs/dlpdes/outputs/heat2d_ms"
    )
    parser.add_argument(
        "--plot_freq",
        type=dict,
    ...
    # integration domain for rank / visualization
    parser.add_argument("--int_grid_n", type=int, default=250)
    parser.add_argument("--int_domain_lowx", type=float, default=0.0)
...
def main():
    ...
    # pipe.load_checkpoint("/home/zhy/Zhou/DLPDEs/dlpdes/outputs/dcei2/_log_model/ckpt_iter_002000.pt")
    ...
    # for i in range(100):
    #     model_old = copy.deepcopy(pipe.trainer.model)
    #     train_center_idx = torch.randperm(pipe.trainer.model.num_centers)[:2].tolist()

    #     print("selected centers:", train_center_idx)
        
    #     local_data = get_local_data_around_centers(pipe.eq,
    #         model=pipe.trainer.model,
    #         center_idx=train_center_idx,
    #         radius=0.3,                  # 你自己调
    #         Nf=500,                      # 局部 interior 点数
