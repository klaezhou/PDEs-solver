...
class LocalBumpTanhNet_time(nn.Module):
    """
    ...
    def _build_grid_centers(self, args) -> torch.Tensor:
        """
        Generate centers only in spatial dimensions.
        ...
        self.space_dim = self.input_dim - 1
        ...
        if center_mins is None:
            ...
            center_mins = [center_min] * self.space_dim

        if center_maxs is None:
            center_max = getattr(args, "center_max", 1.0)
            center_maxs = [center_max] * self.space_dim

        if center_steps is None:
            center_step = getattr(args, "center_step", 0.1)
            center_steps = [center_step] * self.space_dim

        if len(center_mins) != self.space_dim:
            raise ValueError(
                f"len(center_mins)={len(center_mins)} must equal space_dim={self.space_dim}."
            )

        if len(center_maxs) != self.space_dim:
            raise ValueError(
                f"len(center_maxs)={len(center_maxs)} must equal space_dim={self.space_dim}."
            )

        if len(center_steps) != self.space_dim:
            raise ValueError(
                f"len(center_steps)={len(center_steps)} must equal space_dim={self.space_dim}."
            )
        ...
        for d in range(self.space_dim):
            cmin = center_mins[d]
            cmax = center_maxs[d]
            cstep = center_steps[d]

            if cstep <= 0:
                raise ValueError(f"center_steps[{d}] must be positive.")

            if cmax < cmin:
                raise ValueError(f"center_maxs[{d}] must be >= center_mins[{d}].")

            axis_d = torch.arange(
                cmin - self.radius,
                cmax + self.radius + 1e-12,
                cstep
            )
    ...
    def _phi(self, x: torch.Tensor) -> torch.Tensor:
        ...
        phi only depends on spatial variables.
        """
        x_space = x[:, :self.space_dim]          # [B, space_dim]

        diff = x_space[:, None, :] - self.centers[None, :, :]   # [B, N, space_dim]
...
def bump_mlp_time_penultimate_getter(model, x, use_dispatch: bool = True):
    """
